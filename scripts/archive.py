...
start_length = video_cmds.convert_timecode_to_string(start_timecode)
...
def archive():
    ...
    movie_slices = list(filter(lambda x: (".ts" in x) or (
        ".avi" in x)or (".mov" in x)or (".mp4" in x), slice_files))

    print("========== detecting black frames ===========")
    last_slice_sec = video_cmds.detect_black_frames(
        working_directory + "/" + movie_slices[len(movie_slices) - 1],
        working_directory + "/blackdetect",
    )

    total_length = (len(movie_slices) - 1) * file_max_length + \
        last_slice_sec - 2 - start_length
    output_length_string = video_cmds.convert_seconds_to_timecode(total_length)
    end_timecode = output_length_string
    ...
    video_cmds.cmd_popen("rm -rf %s" % (working_directory + "/output.ts"))
    ...
    if (vid):
        video_cmds.aws_cli_publish_vV_sns(
            vid_sns_arn, "ARCHIVE_COMPLETE", vid)
